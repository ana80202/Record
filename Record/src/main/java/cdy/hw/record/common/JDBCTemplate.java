	...
	public static Connection getConnection() {
		...
		try {
			...
			if(conn == null || conn.isClosed() ) {
				...
				// driver.xml 파일 읽어오기,
				//FileInputStream 외부에 있는걸 stream으로 연결해서 사용
				//"driver.xml" 은 프로젝트 바로 밖에 있었기 때문에 쓸수 있었던것
				
				String filePath
					= JDBCTemplate.class.getResource("/cdy/hw/record/sql/driver.xml").getPath();
				//깔끔하게 경로만 얻어와라 getpath()-> c드라이브에서 부터 시작되는 절대 경로가 나옴
				
				System.out.println(filePath);
				//xml-> properties -> resourse 에서 경로 확인 할수 있다!
				//C:\workspace\05_Server\JSPProject02\src\main\java\edu\kh\jsp\sql\driver.xml -(경로)
